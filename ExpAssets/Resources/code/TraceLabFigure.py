...
def load_tracing(path, mirror=False):
	points = []
...
class TraceLabFigure(EnvAgent):
	...
	def __init__(self, import_path=None, manufacture=None, mirror=False, clockwise=True):
		...
		if mirror or not clockwise:
			self.__reverse_segments(self.raw_segments, mirror, clockwise)
	...
	def __import_figure(self, path):
		...
		segments = eval(tlf_info['raw_segments'])
		figure_res = eval(tlf_info['screen_res'])
	...
	def __reverse_segments(self, segments, mirror=True, clockwise=False):
		"""Mirrors the shape across the x-axis and/or reverses its direction.
		...
		Args:
			...
			mirror (boolean): A boolean that indicates if the shape should be flipped
				across the x-axis [mirror = True] or if the presentation of the shape should
				be preserved [mirror = False]
			clockwise (boolean): Whether the direction of the animation should be clockwise
		...
		if mirror:
			...
			for curve, points in segments:
				...
				if curve:
					for p in points:
						flip_pts.append((P.screen_x-p[0], p[1]))
					start, end, ctrl = flip_pts
					pts.append([curve, (start, end, ctrl)])
				else:
					for p in points:
						flip_pts.append((P.screen_x-p[0], p[1]))
					start, end = flip_pts
					pts.append([curve, (start, end)])
			segments = pts

		# Reverse shape direction if needed
		if mirror == clockwise:
			segments.reverse()
			pts = []
			for curve, points in segments:
