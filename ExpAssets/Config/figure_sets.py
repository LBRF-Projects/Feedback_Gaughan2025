...
Originally we were going to use one document per figure-set; now this single
document contains all of them. So first here's the walk-through on how to make
a *single* figure set:
...
To create a figure set, just assign a new FigureSet() object to a variable, and
populate it's name argument.

***IMPORTANT***
The variable name you choose to create the figure set is completely immaterial
and will never again be relevant to the experiment. It is the name you give the
figure set when you create it that's important; this is what you'll type when
setting up a new participant. So, in the following example:

>> new_set = FigureSet("MyFavoriteFigures)

"MyFavorteFigures" is the name of the figure set, NOT new_set.

Then arbitrarily add figures to that set. You can do this in two ways, either en
masse or one at a time; they're identical, it's just up to you whichever you
find cleaner.
...
Which then sort of suggests how different weight values should be added, i.e.:
...
Note that the last item in the add_figures() example has an explicit weight; so
do the first two values, it's just redundant to weight them as 1 each time.
...
The final step is really easy; you just need to ensure you add each created
FigureSet to the variable trace_lab_figure_sets like so:
...
Below is a real-world example that should actually run, provided the figures
listed actually exist in your figures directory.
